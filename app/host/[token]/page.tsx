...
import {HostLogoutButton} from '../../../components/HostLogoutButton';
...
export default function Host({params}:{params:Promise<{token:string}>}){
  ...
  if(mode==='edit')return <main className="container host-page"><header className="host-topbar"><div><div className="eyebrow">ФАКТ / ПАНЕЛЬ ВЕДУЩЕГО</div><div className="host-room">Комната {room.code}</div></div><div className="host-nav"><HostLogoutButton/><ConnectionStatus state={connection}/></div></header><QuestionEditor questions={draft} onChange={setDraft} onRun={()=>setMode('run')} onSave={publish} onExport={exportBackup} onImportFile={importFile} saveError={publishError} dirty={dirty} saving={saving} backupStatus={backupStatus}/></main>;
  if(status==='finished')return <main className="container host-page"><header className="host-topbar"><div className="eyebrow">ВЕДУЩИЙ / ИГРА ЗАВЕРШЕНА</div><div className="host-nav"><HostLogoutButton/><ConnectionStatus state={connection}/></div></header><section className="finish-card panel"><div className="eyebrow">КОМНАТА / {room.code}</div><h1>Игра завершена</h1><p>Все вопросы показаны. Можно сбросить игру и провести её заново.</p><ParticipantStats stats={snapshot.participantStats}/><div className="finish-actions"><button className="btn danger" disabled={busy} onClick={()=>setConfirm('reset')}>Сбросить игру</button></div></section><ConfirmDialog open={confirm==='reset'} title="Сбросить игру?" description="Все ответы будут удалены, игра вернётся в lobby." confirmLabel="Сбросить игру" danger onCancel={()=>setConfirm(null)} onConfirm={()=>{setConfirm(null);void action(()=>repo.resetGame(token))}}/></main>;
  if(status==='lobby'&&room.current_question_id===null)return <main className="container host-page"><header className="host-topbar"><div><div className="eyebrow">ВЕДУЩИЙ / LOBBY</div><div className="host-room">Подготовка к игре</div></div><div className="host-nav"><button className="text-btn" onClick={()=>setMode('edit')}>Редактор</button><HostLogoutButton/><ConnectionStatus state={connection}/></div></header>{loadError&&<div className="form-error">{loadError}</div>}<HostLobby code={room.code} joinUrl={joinUrl} participants={snapshot.participants} busy={busy} dirty={dirty} copyStatus={copyStatus} onCopyCode={()=>void copy(room.code,'Код скопирован')} onCopyLink={()=>void copy(joinUrl,'Ссылка скопирована')} onStart={()=>void action(()=>repo.startQuestion(token))}/></main>;
